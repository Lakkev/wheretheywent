...
test('download CSV of the current view: header carries provenance, rows match the screen', async ({ page }) => {
  ...
  const header = rows[0]!;
  for (const col of ['iso3', 'country_name', 'year', 'metric', 'value', 'source_id', 'source_attribution', 'data_as_of', 'retrieved_at', 'snapshot_id']) expect(header).toContain(col);
...
test('download JSON includes meta with permalink and citations', async ({ page }) => {
  ...
  await waitForApp(page);
  await page.keyboard.press('d');
  const [dl] = await Promise.all([page.waitForEvent('download'), page.getByRole('button', { name: /JSON/ }).click()]);
  const text = await (await dl.createReadStream()).toArray().then((c) => Buffer.concat(c).toString('utf8'));
  const j = JSON.parse(text) as { meta: { permalink: string; citations: Record<string, string>; sources: Record<string, unknown> }; data: { iso3: string; value: number }[] };
...
test('country page cite + download', async ({ page }) => {
  ...
  const [dl] = await Promise.all([page.waitForEvent('download'), page.getByRole('dialog').getByRole('button', { name: /CSV/ }).click()]);
  const text = await (await dl.createReadStream()).toArray().then((c) => Buffer.concat(c).toString('utf8'));
