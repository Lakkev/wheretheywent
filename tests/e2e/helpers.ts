...
export async function waitForApp(page: Page) {
  ...
  await page.waitForSelector('.overlays .ranklist li .val, .nowebgl table tbody tr', { timeout: 30_000 });
}
...
export async function waitForMap(page: Page) {
  await page.waitForFunction(
    () => {
      const m = (window as unknown as { __wtwMap?: { isSourceLoaded: (s: string) => boolean; queryRenderedFeatures: (a?: unknown, o?: unknown) => unknown[] } }).__wtwMap;
      return !!m && m.isSourceLoaded('wtw-countries') && m.queryRenderedFeatures(undefined, { layers: ['wtw-fill'] }).length > 0;
    },
    { timeout: 30_000 },
  );
}
...
export async function mapState(page: Page) {
  return page.evaluate(() => {
    const m = (window as unknown as { __wtwMap?: { getCenter: () => { lng: number; lat: number }; getZoom: () => number } }).__wtwMap;
...
export async function uiSnapshot(page: Page) {
  return page.evaluate(() => {
    ...
    const txt = (s: string) => ov.querySelector(s)?.textContent?.replace(/\s+/g, ' ').trim() ?? null;
    return {
