...
> **`live/*` is ephemeral (S9)**: regenerated by every ETL run, replaced daily, and NOT committed
> to git (the audit trail covers the annual statistics; IDMC/UNHCR retain the live source data).
> A fresh clone has no live files until `npm run etl` runs.
...
| Taiwan `TWN`                                                                                               | Drawn; no UNHCR data at all; WPP population present (name "China, Taiwan Province of China" in WPP) — display policy (c) — see /about/boundaries |
