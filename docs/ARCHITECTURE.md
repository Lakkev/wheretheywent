# ARCHITECTURE — every decision and *why*
...
| `scripts/etl/promote.ts` | Copy passing *groups* into `public/data/v1`, mark failed sources stale, rebuild manifest |
...
`f=1` stays reserved for Phase 2 flow arcs. The map position is only written after a *user* move
